...
// with a message naming the mistake rather than corrupting state quietly. That
// check is cheap and stays on in release builds. The entity map is
// single-goroutine by design and has no mutex; adding one to make it safe from
// other goroutines would remove the reason the rest of the design works.
